...
np.random.seed(1)
num_clients = 100
dir_path = "dataset/emnist/"
train_ratio = 0.8

# Allocate data to users
def generate_emnist(dir_path, num_clients, niid, balance, partition, test):
    ...
    if test:
        ...
        test_set = torchvision.datasets.EMNIST(root=dir_path+"rawdata", split='byclass', train=False, download=True, transform=transform)
        dataset_image = test_set.data
        dataset_label = test_set.targets
    ...
    transform = transforms.ToTensor()
    global_trainset = torchvision.datasets.EMNIST(root=dir_path+"rawdata", split='byclass', train=True, download=True, transform=transform)
    ...
    dataset_image.extend(train_dataset.data)
    dataset_label.extend(train_dataset.targets)
...
if __name__ == "__main__":
    ...
    generate_emnist(dir_path, num_clients, niid, balance, partition, test)