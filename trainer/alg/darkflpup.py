...
class Server(BaseServer):
    ...
    def __init__(self, id, args, dataset, clients, eq_model=None, global_model=None, eq_exits=None):
        ...
        self.dm = copy.deepcopy(self.eq_model[min(self.eq_depths)])
        ...
        for eq_depth in self.eq_depths:
            ...
            optimizer = torch.optim.Adam(params=self.eq_model[eq_depth].parameters(), lr=self.kd_lr)
            lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer=optimizer, gamma=self.gamma)
    ...
    def d_loss(self, gen_latent, y_input, diff):
        ...
        dm_exits_logits = self.eq_policy[min(self.eq_depths)].sf(dm_exits_logits)
    ...
    def y_loss(self, gen_latent, y_input, model, policy, target_probs, exits_num):
        ...
        t_exits_logits = policy.sf(t_exits_logits)
    ...
    def gap_loss(self, diff, t_selected_index_list, eq_depth, t, s, direction='sl'):
        ...
        for t_exit_idx in range(t_exits_num):
            ...
            for s_exit_idx in range(s_exits_num):
                ...
                if direction == 'sl':
                    if self.is_feature: s, t = s_feature, t_feature.detach()
                    else: s, t = s_logits, t_logits.detach()
                    ...
                    gap_loss += weight_t_exits[s_exit_idx]*self.kd_criterion(s,t) * s.shape[0]
    ...
    def progressive_update_model(self, n_iters, diff_g, y_input_g):
        ...
        for _ in range(n_iters):
            ...
            for idx, eq_depth in enumerate(self.eq_depths):
                ...
                s_exits_logits, s_exits_feature = s_model(**self.get_batch(gen_latent, y_input), is_latent=self.is_latent, rt_feature=True, frozen=True)
                sl_Loss += self.gap_loss(diff, t_selected_index_list, eq_depth, (t_exits_logits, t_exits_feature), (s_exits_logits, s_exits_feature))
