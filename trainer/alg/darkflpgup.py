        ...
        for _ in range(n_iters):
            ...
            if self.args.kd_direction == 'sl' or self.args.kd_direction == 'sls':
                for idx, eq_depth in enumerate(self.eq_depths):
                    ...
                    sl_Loss += self.kd_gap * self.gap_loss(diff, y_input, t_selected_index_list, eq_depth, (t_exits_logits, t_exits_feature), (s_exits_logits, s_exits_feature))
            ...
            if self.args.kd_direction == 'ls' or self.args.kd_direction == 'sls':
                for idx, eq_depth in enumerate(reversed(self.eq_depths)):
                    ...
                    s_exits_logits, s_exits_feature = s_model(**self.get_batch(gen_latent, y_input), is_latent=self.is_latent, rt_feature=True, frozen=self.args.kd_frozen)
                    s_exits_logits = s_policy(s_exits_logits)
                    sl_Loss += self.kd_gap * self.gap_loss(diff, y_input, t_selected_index_list, eq_depth, (t_exits_logits, t_exits_feature), (s_exits_logits, s_exits_feature), direction='ls')
