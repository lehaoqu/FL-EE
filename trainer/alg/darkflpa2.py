                ...
                if self.args.slimmable and self.args.slim_block_wise:
                    ...
                    for block_index in range(self.exits_num):
                        ...
                        for slim_ratio in self.args.slim_ratios:
                            ...
                            logits, features, full_token_features = self.model(**batch, is_latent=True, input_block=block_index, stop_exit=block_index)
                            block_slim_logit = logits[0]
                            block_slim_feature = full_token_features[0]
...
class Server(BaseServer):
    ...
    def __init__(self, id, args, dataset, clients, eq_model=None, global_model=None, eq_exits=None):
        ...
        self.clients_embeddings = []
        # == args ==
        self.g_lr, self.g_y, self.g_div, self.g_gap = args.g_lr, args.g_y, args.g_div, args.g_gap
    ...
    def get_conditional(self, ):
        ...
        for t_exit in range(len(self.eq_exits[max(self.eq_depths)])):
            ...
            #     attend_logits += (self.eq_policy[eq_depth](self.eq_model[eq_depth](gen_latent, stop_exit=t_exit, is_latent=self.is_latent)) * self.eq_num[eq_depth] / sum([self.eq_num[eq_depth] for eq_depth in attend_eq]), )
    ...
    def finetune(self):
        ...
        for _ in range(self.s_epoches):
            y_input_g, eps_g = self.get_conditional()
            # gen_latent_g = {}
            # for t_exit in range(len(self.eq_exits[max(self.eq_depths)])):
            #     gen_latent = self.generators[t_exit][0](y_input_g[t_exit], eps_g[t_exit])
            #     gen_latent_g[t_exit] = gen_latent      

            self.train_generators(y_input_g, eps_g)
    ...
    def train_generators(self, y_input_g, eps_g):
        for g in self.generators:
            g[0].to(self.device)
            g[0].train()
        ...
        # == train generators ==
        self.update_generator(y_input_g, eps_g)
    
    def diff(self, y_input, gen_latent):
    ...
    def update_generator(self, y_input_g, eps_g):
        ...
        for i in range(n_iters):
            ...
            for exit_idx in range(len(self.eq_exits[max(self.eq_depths)])):
                ...
                # y_input, eps = y_input_g[exit_idx], eps_g[exit_idx]
