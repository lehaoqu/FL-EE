...
TEXT_SIZE = 14/RATIO
COLORS = ["#E0F7FA", "#B2EBF2", "#80DEEA", "#4DD0E1", "#00BFA5"]
...
def draw(label, dir, title):
    ...
    for idx, d in enumerate(ds):
        plt.scatter(result[int(ALL/len(ds))*(idx):int(ALL/len(ds))*(idx+1), 0], result[int(ALL/len(ds))*(idx):int(ALL/len(ds))*(idx+1), 1], color=COLORS[idx])

    # plt.scatter(result[20:, 0], result[20:, 1], color='red')
...
draw(20, './imgs/generator/darkflpg/', f'Pseudo latent Visualization Label: {20}')

draw(50, './imgs/generator/darkflpg/', f'Pseudo latent Visualization Label: {50}')